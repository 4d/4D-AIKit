...
//  MARK: check that onResponse and onError work too

cs:C1710._TestSignal.me.init()

$helper:=$client.chat.create("You are a helpful assistant."; {stream: True:C214; \
onData: Formula:C1597(cs:C1710._TestSignal.me.pushChunk($1)); \
...
If (Asserted:C1132(Bool:C1537($streamResult.success); "Cannot complete chat : "+JSON Stringify:C1217($streamResult)))
	...
	If (Asserted:C1132(($streamResults#Null:C1517) && ($streamResults.length>0); "No chunk received for streaming"))
		...
		$message:=$streamResults.extract("choice").extract("delta").extract("text").join("")
