...
var $listParams:=cs:C1710.OpenAIFilesListParameters.new({\
purpose: "fine-tune"; \
limit: 5; \
order: "desc"\
})
...
If (Asserted:C1132(Bool:C1537($filteredListResult.success); "Cannot list files with parameters: "+JSON Stringify:C1217($filteredListResult)))
	...
	If (Asserted:C1132($filteredListResult.files#Null:C1517; "Filtered files collection must not be null"))
		...
		ASSERT:C1129($filteredListResult.files.length<=5; "Should not return more than 5 files")
...
If (Length:C16($uploadedFileId)>0)
	...
	var $contentResult:=$client.files.content($uploadedFileId)
...
var $invalidFileResult:=$client.files.create(Null:C1517; "fine-tune")
ASSERT:C1129(Not:C34(Bool:C1537($invalidFileResult.success)); "Should fail with null file")

var $invalidPurposeResult:=$client.files.create($testFile; "")
ASSERT:C1129(Not:C34(Bool:C1537($invalidPurposeResult.success)); "Should fail with empty purpose")

var $invalidRetrieveResult:=$client.files.retrieve("")
ASSERT:C1129(Not:C34(Bool:C1537($invalidRetrieveResult.success)); "Should fail with empty file ID")

var $invalidDeleteResult:=$client.files.delete("")
ASSERT:C1129(Not:C34(Bool:C1537($invalidDeleteResult.success)); "Should fail with empty file ID for deletion")

// MARK:- Test different file purposes
var $assistantsFile:=$testDataFolder.file("assistants_test.txt")
$assistantsFile.setText("This is a test file for assistants API.")

var $assistantsUpload:=$client.files.create($assistantsFile; "assistants")

If (Asserted:C1132(Bool:C1537($assistantsUpload.success); "Cannot upload assistants file: "+JSON Stringify:C1217($assistantsUpload)))
	
	ASSERT:C1129($assistantsUpload.file.purpose="assistants"; "Purpose should be 'assistants'")
	
	// Clean up
	var $cleanupResult:=$client.files.delete($assistantsUpload.file.id)
...
If ($testFile.exists)
	$testFile.delete()
End if 
If ($assistantsFile.exists)
	$assistantsFile.delete()
End if 
